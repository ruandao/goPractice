...
func main() {
	...
	http.HandleFunc("/getImage", serveImage2)
...
func serveImage2(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		values, err := url.ParseQuery(r.URL.RawQuery)
		if err != nil {
			fmt.Fprint(w, err)
		...
		response, err := http.Get("http://" + masterLocation + "/get?state=finished&id=" + values.Get("id"))
		if err != nil || response.StatusCode != http.StatusOK {
			w.WriteHeader(http.StatusBadRequest)
			fmt.Fprint(w, "Error:", err)
			return
